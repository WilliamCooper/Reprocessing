...
\global\addressee={START08 reprocessing file} % >>change "File" to the "To:" name desired

\begin{tabular}{ll}
\textsf{\textsc{\textcolor{blue}{To:}}}  & \the\addressee\tabularnewline
\textsf{\textsc{\textcolor{blue}{From:}}}  & Al Cooper\tabularnewline
\textsf{\textsc{\textcolor{blue}{Subject:}}}  & vertical wind for START08\tabularnewline
\end{tabular}
...
FCap1 <- c("Vertical wind, START08 flight 5. The blue trace is WIC; the red trace (WICS) is the same measurement after application of 60-s smoothing. The dashed orange line shows the mean value for the flight.", "Residual error as defined in (1) as a function of Mach number. The dashed orange line is the mean value, 0.424 deg.")
...
Review of START08 measurements shows that, when the "standard" sensitivity coefficients as given in the Processing Algorithms technical note are used, there is often a significant
offset in vertical wind and there is much variability from flight
to flight and even within flights. For example, for START08 flight
5, Fig.~\ref{fig:S08vw-rf051} shows the measurements of vertical wind.
...
For START08 and all projects before SPRITES-II, there is the problem
...
biases. Such flights may extend through atmospheric regions
...
of the difference for START08 flight 5 of \Sexpr{round(mean (Data$GGVS[Data$RF==5]-Data$VSPD_A[Data$RF==5], na.rm=TRUE), 2)}$\pm$\Sexpr{round(sd (Data$GGVS[Data$RF==5]-Data$VSPD_A[Data$RF==5], na.rm=TRUE), 2)}\,m/s. The
...
<<S08vw-rf05, include=TRUE, fig.cap=FCap1>>=
...
This memo will use measurements from rf01--rf18. Some data restrictions
...
of attack. For START08, there are frequent descents to low level followed
...
in plots like Fig.~\ref{fig:S08summarize-fit} below. In addition, because
...
by (\ref{eq:alpha-star}) is shown in Fig.~\ref{fig:S08summarize-fit}.
...
<<S08summarize-fit, include=TRUE, fig.cap="Calculated value of angle of attack vs the reference value used in the fit.">>=
...
is needed to convert from degrees to radians. Figure~\ref{fig:S08new-vw} repeats Fig.~1
...
START08 research flights are appended to this memo, beginning with
Fig.~\ref{fig:S08all-flights1}. Gaps in these plots often arise because the
...
<<S08new-vw, include=TRUE, fig.cap="As for Fig.\ 1 but adding the new variable WIX for the vertical wind.">>=
...
\{\Sexpr{cf[1]}, \Sexpr{cf[2]}, \Sexpr{cf[3]}\} for START08.
...
\begin{tabular}{ll}
\textsf{\textsc{\textcolor{blue}{Project:}}}  & WI-START08\tabularnewline
\textsf{\textsc{\textcolor{blue}{Archive package:}}}  & WI-START08.zip\tabularnewline
\textsf{\textsc{\textcolor{blue}{Contains:}}}  & attachment list below\tabularnewline
\textsf{\textsc{\textcolor{blue}{Program:}}}  & AKRDforSTART08.Rnw\tabularnewline
\textsf{\textsc{\textcolor{blue}{This document:}}}  & AKRDforSTART08.pdf\tabularnewline
\textsf{\textsc{\textcolor{blue}{Workflow:}}} & WorkflowFindAKRDcal.pdf\tabularnewline
\textsf{\textsc{\textcolor{blue}{Original Data:}}}  & /scr/raf\_data/START08/START08rf01.nc, etc \tabularnewline
...
\attachm{AKRDforSTART08.Rnw\\
AKRDforSTART08.pdf\\WorkflowFindAKRDcal.pdf\\
SessionInfo} %\cc{first attachment\\second\\3rd att}%\attach{attachment}%\attachm{first\\second} %\cc{first attachment\\second\\3rd att}

<<S08all-flights, include=TRUE, fig.cap="Recalculated vertical wind WIX and, with 60-s smoothing, WIXS.">>=
...
system (sprintf("zip %s.zip AKRDforSTART08.Rnw AKRDforSTART08.pdf WorkflowFindAKRDcal.pdf SessionInfo", thisFileName))
