...
nplots=c(1, 3:17, 19:20, 23)    # project default
...
## make the html file, but only for SavePlotsToFiles == 1
  if (SavePlotsToFiles == 1) {
    ...
    syscmd <- paste ("Rscript -e 'commandArgs(TRUE);knitr::spin (\"ReviewSTART08.R\",",
                     "format=\"Rmd\")'", sprintf ("%s %s 3 ", Flight, npsl), sep=' ')
    system (syscmd, wait=TRUE)
    system (sprintf ("mv ReviewSTART08.html %s", plothtml))
  }
} else {
