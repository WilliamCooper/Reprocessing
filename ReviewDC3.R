...
cx[2] = 22.856
cx[3] = 0.23639
rad <- (c[2]**2-4.*(c[1]-Data$RTF1)*c[3])**0.5
Volts <- (-c[2] + rad) / (2.*c[3])
Data$RTF1 <- cx[1]+cx[2]*Volts+cx[3]*Volts**2
if (UseNewCal) {Data$ATF1 <- AirTemperature (Data$RTF1, Data$PSFC, Data$QCFC)}
# c[1] = -83.566
# c[2] = 22.704
# c[3] = 0.39350
# cx[1] = -82.739
...
## make the html file, but only for SavePlotsToFiles == 1
  if (SavePlotsToFiles == 1) {
    ...
    syscmd <- paste ("Rscript -e 'commandArgs(TRUE);knitr::spin (\"ReviewDC3.R\",",
                     "format=\"Rmd\")'", sprintf ("%s %s 3 ", Flight, npsl), sep=' ')
    system (syscmd, wait=TRUE)
    system (sprintf ("mv ReviewDC3.html %s", plothtml))
  }
} else {
