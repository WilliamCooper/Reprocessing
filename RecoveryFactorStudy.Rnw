...
was differentiated to find $\alpha(X)$. Because $M^{2}=V^{2}/(\gamma RT_{0})=2C_{v}X/(RT_{0})$
and the fit also gives $T_{0}$, the resulting function $\alpha(X)$
can be transformed to $\alpha^{\prime}(M)=\alpha(X=M^{2}RT_{0}/(2C_{v})$. 
