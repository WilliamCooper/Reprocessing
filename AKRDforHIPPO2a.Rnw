...
Data$VSPD_G <- 0.06 + Data$VSPD_G/1.02  ## removes cal coefficients in processing
D3 <- Data[Data$RF == 3, ]
D5 <- Data[Data$RF == 5, ]
D3 <- D3[!is.na(D3$Time), ]
...
avionic-system GPS. The former is updated using the pressure altitude as reference,
which might introduce long-distance biases, especially in
...
\item A few other regions needed to be excluded because it appeared that ADIFR was questionable, perhaps
because there were blockages in the lines or apertures.
\end{enumerate}
...
\hline 
\hline
tf02 & 172115 & 213214 \tabularnewline
\hline
...
\hline
rf05 & 21:30:00 & 271217\tabularnewline
\hline
rf06 & 212555 & 285500\tabularnewline
\hline
rf07 & 211308 & 285407\tabularnewline
\hline
rf08 & 230239 & 312526\tabularnewline
\hline
rf09 & 201032 & 274108\tabularnewline
\hline
rf10 & 191046 & 262346\tabularnewline
\hline
rf11 & 184042 & 232044\tabularnewline
\hline
\end{tabular}
...
\Sexpr{round(cf[2], 3)}, \Sexpr{round(cf[3], 3)}\}. A comparison of the angle of attack produced by (1)
...
deviation for this fit was reduced slightly (about 10\%) from the fit that used only the first term,
so it appears worthwhile
...
significant (>0.01) further reduction in the standard deviation of the residuals
so it does not appear useful to include more complex terms in the fit.
...
<<all-flights, include=TRUE, fig.cap="recalculated vertical wind WICN and, with new coefficients, WIN">>=

for (i in 1:11){
  plotWAC(Data[Data$RF == i, c("Time", "WIN", "WINS")])
...
\textsf{\textsc{\textcolor{blue}{Original Data:}}} & /scr/raf\_data/\Sexpr{Project}/\Sexpr{Flight}.nc, etc \tabularnewline
...
\attachm{AKRDforHIPPO2a.Rnw\\AKRDforHIPPO2a.pdf\\SessionInfo}
