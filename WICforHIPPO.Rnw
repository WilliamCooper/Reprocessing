...
projects. This will be discussed further in the ``Suggested choice''
section below.
...
\hline 
HIPPO-5 & Aug/Sept 2011 & 5.044 & 9.746 & 9.567\tabularnewline
\hline 
...
d[is.na(d)] <- 0
Data$WIXF <- Data$WIX - signal::filter( bf, d)
...
\section{Suggested choice}

For this reason, we propose a different solution for HIPPO-4 and HIPPO-5:
\begin{enumerate}
\item Determine a fit using only a two-coefficient fit, to obtain the best
estimate of the second coefficient relating ADIFR/QCF to AKRD: AOAREF
= c{[}1{]} + c{[}2{]}ADIFR/QCF. It appears preferable to use the fit
to DC3-TEST data because this program included several speed runs
suitable for determining these coefficients. Therefore, the suggested
fit coefficients for HIPPO-4/5 are \{5.389, 20.172, 0\}.
\item Use a high-pass filter with long time constant, perhaps 600 s, to
adjust the resulting vertical wind to remove the offset.
\end{enumerate}
This avoids the danger that a fit like those shown for HIPPO-4 and
HIPPO-5 will distort the sensitivity to the radome pressures from
periods of faulty radome performance, keep what appears to be a reasonable
sensitivity to the ratio ADIFR/QCF, and ensure that the resulting
vertical wind will remain near zero in the mean. A 600~s time constant
will correspond, at a flight speed of 180 m/s, to a wavelength of
at least 100 km, so only vertical motion persisting at least this
far will be affected.
...
HIPPO-4 and HIPPO-5. See the sequence of figures starting with Fig.~\ref{fig:all-flight-results}.
...
for (i in 1:13) {
  ...
  title (sprintf ("HIPPO-5 flight %d", i))
}
...
for (i in 1:12) {
  plotWAC(Data[Data$RF == i, V])
  title (sprintf ("HIPPO-4 flight %d", i))
}
