...
UseNewCal <- TRUE   ##!! the following may not be right for DC3-TEST, 
                    ##!! but DC3-TEST immediately followed H-4 so maybe?
c <- vector("numeric", 3)
cx <- vector("numeric", 3)
...
if (UseNewCal) {
  ...
  Data$ATH3 <- AirTemperature (Data$RTH3, Data$PSFC, Data$QCFC)
}
c[1] = -89.148
c[2] = 25.709
c[3] = -0.0513
...
if (UseNewCal) {
  ...
  Data$ATH4 <- AirTemperature (Data$RTH4, Data$PSFC, Data$QCFC, probe='HARCOB')
}
Data$ATX <- Data$ATH3  ## -- needed for HIPPO-4/5, probably DC3-TEST also
...
## make the html file, but only for SavePlotsToFiles == 1
  if (SavePlotsToFiles == 1) {
    ...
    syscmd <- paste ("Rscript -e 'commandArgs(TRUE);knitr::spin (\"ReviewDC3-TEST.R\",",
                     "format=\"Rmd\")'", sprintf ("%s %s 3 ", Flight, npsl), sep=' ')
    system (syscmd, wait=TRUE)
    system (sprintf ("mv ReviewDC3-TEST.html %s", plothtml))
  }
} else {
